...
class ClusterAnalysisGUI:
    def __init__(self, master):
        ...
        try:
            ...
            self.cluster_labels = None  # Add this line
            self.current_plot = None    # Add this line
            ...
            # Create frames for clustering methods BEFORE initializing tabs
    ...
    def save_plot(self):
        ...
        try:
            if not hasattr(self, 'current_plot') or self.current_plot is None:
    ...
    def export_plot_data(self):
        ...
        try:
            ...
            if filename:
                ...
                self.status_var.set(f"Plot data exported to {filename}")
                self.logger.info(f"Plot data exported to {filename}")
                
        except Exception as e:
            self.logger.error(f"Failed to export plot data: {str(e)}")
            messagebox.showerror("Error", f"Failed to export plot data: {str(e)}")
    ...
    def preview_file(self):
        ...
        try:
            ...
            # Update preview text
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, "File Preview:\n\n")
            self.preview_text.insert(tk.END, str(df))
            
            self.logger.info("File preview updated successfully")
            
        except Exception as e:
            self.logger.error(f"Failed to preview file: {str(e)}")
    ...
    def create_kmeans_frame(self):
        ...
        try:
            ...
            self.logger.info("K-means frame created successfully")
    ...
    def create_dbscan_frame(self):
        ...
        try:
            ...
            self.logger.info("DBSCAN frame created successfully")
    ...
    def create_hierarchical_frame(self):
        ...
        try:
            self.hierarchical_frame = ttk.LabelFrame(self.cluster_tab, text="Hierarchical Clustering Parameters")
            ...
            self.logger.info("Hierarchical clustering frame created successfully")
    ...
    def estimate_epsilon(self):
        ...
        try:
            ...
            neighbors_fit = neighbors.fit(self.normalized_data)
            distances, _ = neighbors_fit.kneighbors(self.normalized_data)
            ...
            if kneedle.knee is not None:
                estimated_epsilon = distances[kneedle.knee]
                ax.axhline(y=estimated_epsilon, color='r', linestyle='--',
                          label=f'Estimated ε = {estimated_epsilon:.3f}')
                ax.legend()
                
                # Update epsilon value
                self.eps.set(estimated_epsilon)
            
            # Show plot
            plt.show()
            
            self.logger.info(f"Estimated epsilon: {estimated_epsilon:.3f}")
    ...
    def show_dendrogram(self):
        ...
        try:
            ...
            self.logger.info("Displayed dendrogram")
            
        except Exception as e:
            self.logger.error(f"Failed to show dendrogram: {str(e)}")
            messagebox.showerror("Error", f"Failed to show dendrogram: {str(e)}")
